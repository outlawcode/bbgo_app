...
const RootNavigation = (props) => {
  ...
  // PushNotification configuration disabled
  // Use Firebase Cloud Messaging instead

  useEffect(() => {
    LogBox.ignoreAllLogs();
    // Đảm bảo các hàm liên quan đến Firebase chỉ được gọi sau khi đã khởi tạo
    import('../app/utils/firebaseInit').then(({ isFirebaseInitialized, initializeFirebase }) => {
      if (isFirebaseInitialized()) {
        ...
        initializeFirebase().then(success => {
          if (success) {
            ...
            console.warn('Không thể khởi tạo Firebase.');
          }
        });
      }
    })
...
const EntryPoint = () => {
  ...
  const appInfo = {
    appId: Platform.OS === 'android' ? 'com.brs.bbherb' : 'id6744519551', // Your app url in play store or app store
    appName: 'BBHerb', // Your app name
