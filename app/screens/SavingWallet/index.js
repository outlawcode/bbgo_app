...
import TransferMoneyScreen from "app/screens/SavingWallet/TransferMoneyScreen";

function SavingWalletScreen(props) {
	...
	return (
		<View style={tw`flex bg-white min-h-full content-between`}>
			<View style={tw`bg-pink-500 px-3 pt-12 pb-3`}>
				...
				<View style={tw`bg-pink-500 pb-2`}>
					<View style={tw`flex items-center`}>
						<Text style={tw`text-white mb-3 font-medium text-lg`}>Ví tiết kiệm</Text>
						<Text style={tw`text-white text-xs mb-1`}>Số dư ví</Text>
						<Text style={tw`text-white font-bold text-4xl`}>{currentUser && formatVND(currentUser.savingWallet)}</Text>
					</View>
				</View>
				...
				<View style={tw`flex flex-row items-center justify-between mt-5`}>
					<TouchableOpacity
						...
						onPress={() => props.navigation.navigate('Modal', {
							content: <CreateSavingScreen
								navigation={props.navigation}
								backScreen={'SavingWallet'}
							/>
						})}
					>
						<Icon name={"plus-circle"} size={24} style={tw`text-white mr-1`} />
						<Text style={tw`text-white`}>Mở tiết kiệm</Text>
					</TouchableOpacity>
					<TouchableOpacity
						style={tw`flex flex-row items-center`}
						onPress={() => props.navigation.navigate('Modal', {
							content: <TransferMoneyScreen
								...
								backScreen={'SavingWallet'}
								wallet={'Ví tiết kiệm'}
								balance={currentUser && currentUser.savingWallet}
							/>
						})}
					>
...
export default SavingWalletScreen;
