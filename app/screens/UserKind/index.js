...
function UserKindScreen(props) {
	...
	return (
		<View style={tw`flex bg-white min-h-full content-between`}>
			<View style={tw`bg-orange-500 px-3 pt-12 pb-3`}>
				<View style={tw`flex flex-row items-center justify-between`}>
					...
					>
						<Icon name={"crown"} size={24} style={tw`text-white mr-1`} />
						<Text style={tw`text-white`}>Nâng cấp</Text>
					</TouchableOpacity>
				</View>

				<View style={tw`bg-orange-500 pb-2`}>
					<View style={tw`flex items-center`}>
						<Text style={tw`text-white mb-3 font-medium text-lg`}>Loại tài khoản</Text>
						{currentUser && currentUser.userKind ?
							<>
								<Text
									style={tw`text-white font-bold text-3xl mb-2`}>{currentUser && currentUser.userKind ? currentUser.userKind.name : 'Chưa có'}</Text>
							</>
							:
							<View style={tw`bg-white opacity-50 px-3 py-2 rounded`}>
								<Text style={tw`text-green-600 text-xl font-medium`}>Tài khoản thường</Text>
							</View>
						}
			...
			{currentUser && !currentUser.userKind &&
				<View style={tw`my-3 mx-3 bg-orange-50 border border-orange-200 flex items-center p-5 rounded`}>
					<Text style={tw`mb-3 text-center`}>
						Nâng cấp tài khoản để hưởng nhiều ưu đãi và chiết khấu khi mua hàng của thành viên VIP.
					</Text>
					<TouchableOpacity
						style={tw`bg-orange-500 px-3 py-2 rounded flex items-center flex-row`}
						onPress={() => props.navigation.navigate('Modal', {
							content: <UpgradeAccount navigation={props.navigation} backScreen={'UserKind'}/>
						})}
					>
						<Icon name={"crown"} style={tw`text-white mr-2`} size={20}/>
						<Text style={tw`text-white font-bold`}>Nâng cấp ngay</Text>
					</TouchableOpacity>
				</View>
			}
			<ScrollView
					...
					<View style={tw`px-3`}>
						<View style={tw`flex flex-row items-center justify-between mb-3 pt-5`}>
							<View style={tw`flex flex-row items-center`}>
								...
								<Text style={tw`font-medium text-gray-500`}>Lịch sử nâng cấp</Text>
							</View>
