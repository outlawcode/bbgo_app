...
function AffWalletScreen(props) {
	...
	return (
		<View style={tw`flex bg-white min-h-full content-between`}>
			...
			<View style={tw`bg-yellow-400 px-3 pt-12 pb-3`}>
				<View style={tw`flex flex-row items-center justify-between`}>
					<TouchableOpacity
						onPress={() => props.navigation.goBack()}
					>
						<Icon name={"chevron-left"} size={30} style={tw`text-gray-800`} />
					</TouchableOpacity>
					<TouchableOpacity
						...
						onPress={() => props.navigation.navigate('Modal', {
							content: <WithdrawBankScreen
								...
								backScreen={'AffWallet'}
								wallet={'affWallet'}
							/>
						})}
						//disabled={currentUser && currentUser.rewardWallet <= 0}
					>
						<Icon name={"tray-arrow-down"} size={24} style={tw`text-gray-800 mr-1`} />
						<Text style={tw`text-gray-800`}>Rút tiền</Text>
					</TouchableOpacity>
				</View>

				<View style={tw`bg-yellow-400 pb-2`}>
					<View style={tw`flex items-center`}>
						<Text style={tw`text-gray-800 mb-3 font-medium text-lg`}>Ví hoa hoa hồng</Text>
						<Text style={tw`text-gray-800 text-xs mb-1`}>Số dư ví</Text>
						<Text style={tw`text-gray-800 font-bold text-4xl`}>{currentUser && formatVND(currentUser.affWallet)}</Text>
					</View>
				</View>
			</View>
			<ScrollView
					...
					<View style={tw`px-3`}>
						<View style={tw`flex flex-row items-center justify-between mb-3 pt-5`}>
							<View style={tw`flex flex-row items-center`}>
								{/*<Icon name={"history"} size={18} style={tw`mr-1 text-blue-500`} />*/}
...
export default AffWalletScreen;
