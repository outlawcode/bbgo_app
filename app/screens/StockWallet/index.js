...
function StockWalletScreen(props) {
	...
	useEffect(() => {
		if (isFocused) {
			async function getPackageInfo() {
				...
				axios({
					method: 'get',
					url: `${apiConfig.BASE_URL}/customer/transaction`,
					params: {
						limit: 0,
						page,
				...
				<View style={tw`flex flex-row items-center justify-between`}>
					...
					<TouchableOpacity
						...
						onPress={() => props.navigation.navigate('Modal', {
							content: <CreateInvestmentScreen
								currentUser={currentUser}
								settings={settings}
								navigation={props.navigation}
								backScreen={'StockWallet'}
							/>
						})}
					>
						<Icon name={"tray-arrow-up"} size={24} style={tw`text-white mr-1`} />
						<Text  style={tw`text-white`}>Đầu tư</Text>
					</TouchableOpacity>
				</View>
				<View style={tw`bg-purple-500 pb-2`}>
					<View style={tw`flex items-center`}>
						<Text  style={tw`text-white mb-3 font-medium text-lg`}>Ví cổ phần</Text>
						<Text  style={tw`text-white text-xs mb-1`}>Số dư ví</Text>
						<Text  style={tw`text-white font-bold text-4xl`}>{currentUser && formatVND(currentUser.stock_wallet)}</Text>
					</View>
				</View>
			</View>
			<ScrollView
...
export default StockWalletScreen;
