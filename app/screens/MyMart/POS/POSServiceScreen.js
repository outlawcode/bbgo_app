...
function POSServiceScreen(props) {
    ...
    useEffect(() => {
        props.navigation.setOptions({
            title: 'SME POS Service',
            headerStyle: {
                backgroundColor: '#2e65a5',
            },
            headerTintColor: '#fff',
            headerLeft: () => (
                <TouchableOpacity
    ...
    useEffect(() => {
        if (Number(searchQuery.length) > 2) {
            ...
            async function getData() {
                ...
                axios({
                    method: 'get',
                    url: `${apiConfig.BASE_URL}/member/services/search`,
                    params: {
                        query: searchQuery,
                        serviceAddress: userShop ? [userShop.id] : [],
                    },
                    headers: { Authorization: `Bearer ${token}` }
                }).then(function(response) {
                    if (response.status === 200) {
    ...
    async function handleCreateOrder() {
        ...
        if (tabIndex !== -1) {
            ...
            if (itemsOrder.length > 0) {
                ...
                return axios({
                    method: 'post',
                    url: `${apiConfig.BASE_URL}/member/order/create-offline-service`,
                    data: {
                        items: JSON.stringify(itemsOrder),
                        note,
                        restaurantId: userShop && userShop.id
                    },
                    headers: {Authorization: `Bearer ${token}`}
                }).then(function (response) {
                    ...
                    >
                        {tabs && tabs.map((el) => (
                            <TouchableOpacity
                                ...
                                style={tw`${activeTab === el ? 'bg-blue-600' : 'bg-white'} rounded-tl-md rounded-tr-md py-3 px-4`}
                            >
                        ...
                        <View style={tw`pb-32`}>
                            {loading ? <ActivityIndicator /> :
                                notFound ? <View style={tw`flex items-center`}>
                                        ...
                                        <Text style={tw`text-gray-500`}>Không tìm thấy dịch vụ</Text>
                                    </View>
                                    :
                                        ...
                                        <View style={tw`py-3 border-b border-gray-100`}>
                                            ...
                                            >
                                                ...
                                                <View style={tw`flex items-center justify-between flex-row`}>
                                                    <Text>Mã: {el.id}</Text>
                                                    <Text style={tw`font-medium text-blue-600`}>{formatVND(el.price)}</Text>
                                                </View>
                                            </TouchableOpacity>
                                        </View>
                                    ))
                            ...
                            <View style={tw`pb-52`}>
                                {item.items.length > 0 ?
                                    ...
                                    <View style={tw`flex items-center m-5`}>
                                        <Text>Vui lòng chọn dịch vụ cho vào đơn hàng</Text>
                                    </View>
                                }
                            </View>
                        </ScrollView>
                        ...
                        >
                                ...
                                <View>
                                    {settings && Number(settings.vat_percent) > 0 &&
                                        <View style={tw`flex flex-row items-center justify-between py-2 border-b border-gray-100 mb-2`}>
                                            <Text>VAT ({settings.vat_percent}%)</Text>
                                            <Text style={tw`text-gray-700`}>+{formatVND(calculator(item.items).totalAmountVAT)}</Text>
                                        </View>
                                    }
                                    <View style={tw`flex flex-row items-center justify-between py-2 border-b border-gray-100 mb-2`}>
                                        <Text>Tạm tính</Text>
                                        <Text>{formatVND(calculator(item.items).totalAmount)}</Text>
                                    </View>
                                </View>
                            }
                            ...
                            <View style={tw`flex items-center flex-row justify-between`}>
                                ...
                                >
                                    ...
                                    <Text style={tw`text-base font-bold text-blue-600`}>{formatVND(calculator(item.items).lastTotalAmount)}</Text>
                                </TouchableOpacity>
                                <View style={tw`flex items-center flex-row`}>
                                    ...
                                    <TouchableOpacity
                                        style={tw`bg-blue-600 p-2 rounded border border-blue-600 flex items-center flex-row`}
...
export default POSServiceScreen;
