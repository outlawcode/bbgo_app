...
function CustomerInformationFood(props) {
	...
	function handleCheckout(values) {
		props.navigation.navigate('PaymentMethodFood', {
			checkoutData: {
				...values,
								...
								{showDetail &&
									<View>
										...
										<View
											style={tw`flex flex-row items-center justify-between mb-2 border-b border-gray-100 pb-2`}>
											<Text>VAT</Text>
											<Text>{formatVND(state.vatAmount)}</Text>
										</View>
									</View>
								}
								<View style={tw`flex flex-row items-center justify-between mb-1`}>
									<Text  >Tổng tiền</Text>
									<Text style={tw`text-green-600 text-base font-bold`}>{formatVND(state.totalAmount - state.discount + state.vatAmount)}</Text>
								</View>
							</View>
...
export default CustomerInformationFood;
