...
function ClassroomRollup(props) {
    ...
    useEffect(() => {
        if (isFocused) {
            async function getData() {
                axios({
                    method: 'get',
                    url: `${apiConfig.BASE_URL}/classroom/${id}`,
    ...
    async function handleRollup() {
        ...
        axios({
            method: 'post',
            url: `${apiConfig.BASE_URL}/member/classroom/rollup/${id}`,
            data: {},
            headers: {Authorization: `Bearer ${Token}`},
        }).then(function (response) {
            showMessage({
                message: `Đã điểm danh vào lớp ${result && result.name}`,
                type: 'success',
                icon: 'success',
                duration: 3000,
            });
                            ...
                            <View style={tw`mb-3 bg-white p-3`}>
                                {error ?
                                    <View style={tw`flex items-center my-5`}>
                                        ...
                                        <Text  style={tw`text-gray-600`}>Không tìm thấy thông tin lớp học</Text>
                                    </View>
                                    :
                                    <View style={tw`py-10`}>
                                        <View style={tw`flex items-center mb-5`}>
                                            <Text style={tw`text-lg font-bold text-gray-700`}>{result && result.name}</Text>
                                        </View>
                                        <View style={tw`flex items-center mb-2`}>
                                            <Text>{result && result.address}</Text>
                                        </View>
...
export default ClassroomRollup;
